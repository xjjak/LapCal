...
#     print(model.summary())
#     var_sizes = [
#         np.prod(list(map(int, v.shape))) * v.dtype.size
...
#                 tf.keras.layers.LSTM(22), input_shape=(seq_length, 3)
#             ),  # output_shape=(batch, 44)
...
def load_data(train_data_path, valid_data_path, test_data_path, seq_length):
    ...
    data_loader.format()
    return (
        data_loader.train_len,
        data_loader.train_data,
        data_loader.valid_len,
        data_loader.valid_data,
        data_loader.test_len,
...
def train_net(
    ...
    confusion = tf.math.confusion_matrix(
        labels=tf.constant(test_labels), predictions=tf.constant(pred), num_classes=4
    )
    print(confusion)
...
if __name__ == "__main__":
    ...
    train_len, train_data, valid_len, valid_data, test_len, test_data = load_data(
        "./_datasets/split/train",
        "./_datasets/split/valid",
        "./_datasets/split/test",
        seq_length,
    )
